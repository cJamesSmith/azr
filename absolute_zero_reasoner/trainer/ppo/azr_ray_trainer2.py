...
class CodeIORayPPOTrainer(ReasonRLRayPPOTrainer):
    ...
    def create_dataloader(self, sampled_training_dataset: List[Dict], output_path: str = '/home/aiops/chenxw/CURE/data/') -> None:
        ...
        **Test Input:**\n```input here```\n\n**Test Output:**\n```output here```\n\n**Explanation:**\n\nexplanation here.\n <|im_end|>\n<|im_start|>Assistant: """
        ...
        code_training_data = []
        case_training_data = []
        for data in sampled_training_dataset:
            question = data.pop('question')

            # code dataset
            content = Template(system_prompts).render(language = "python", special_requirements = special_requirements, problem = question)
            item = {
                "data_source": data.pop('dataset'),
                "prompt": [{
                    "role": "user",
                    "content": content,
                }],
            ...
            item['generated_code'] = []
            code_training_data.append(item.copy())
            item.pop("generated_code")
            item['method'] = "case"
            
            n_example = len(data["example_input"])
            example_input = ", ".join([repr(item) for item in data['example_input']])
            example_output = ", ".join([repr(item) for item in data['example_output']])
            if n_example == 0:
                example_intro = """ """
            if n_example == 1:
            ...
            content = Template(system_case_prompts).render(problem = question, example_intro = example_intro)
            item["prompt"] = [{
                "role": "user",
                "content": content,
            }]
            
            max_k = min(self.config.cure.sample.max_ground_truth_test, len(data["test_input"]))
            item["num_ground_truth_test"] = max_k 
            item["all_case_input"] = (data["test_input"][:max_k]).copy()
            item["all_case_output"] = (data["test_output"][:max_k]).copy()
            item["case_input"] = []
            item["case_output"] = []
            item["case_text"] = []
            case_training_data.append(item.copy())

        df = pd.DataFrame(code_training_data)
        df.to_parquet(output_path+"train.parquet")
        code_training_dataset = RLHFDataset(
            parquet_files=output_path+"train.parquet",
            tokenizer=self.tokenizer,
            prompt_key=self.config.data.prompt_key,
            max_prompt_length=self.config.data.max_prompt_length,
            filter_prompts=True,
            return_raw_chat=self.config.data.get('return_raw_chat', False),
            truncation='error',
            extra_source_key=f"{item['data_source']}_train"
        )
        sampler = SequentialSampler(code_training_dataset)

        df = pd.DataFrame(case_training_data)
        df.to_parquet(output_path+"train.parquet")
        case_training_dataset = RLHFDataset(
            parquet_files=output_path+"train.parquet",
            tokenizer=self.tokenizer,
            prompt_key=self.config.data.prompt_key,
            max_prompt_length=self.config.data.max_prompt_length,
        ...
        sampler = SequentialSampler(case_training_dataset)
        return DataLoader(
            dataset=code_training_dataset,
            batch_size=self.config.data.train_batch_size,
            drop_last=True,
            collate_fn=collate_fn,
            sampler=sampler
        ), DataLoader(
            dataset=case_training_dataset,
            batch_size=self.config.data.train_batch_size,
            drop_last=True,
            collate_fn=collate_fn,
            sampler=sampler
    ...
    def extract_code(self, full_output):
    ...
    def extract_test_cases(self, full_output):
        def modify(c):
    ...
    def fit(self):
        ...
        with open("/home/aiops/chenxw/CURE/data/CodeContests_train.json", 'r') as file:
                        raw_training_dataset = json.load(file)
        PrettyPrinter.status("DATA", f"Loaded {len(raw_training_dataset)} seed entries", "success")
        random_select_num = min(self.config.cure.sample.random_select_num, len(raw_training_dataset))
        sampled_training_dataset = random.sample(raw_training_dataset, random_select_num)

        code_dataloader, case_dataloader = self.create_dataloader(sampled_training_dataset)
        for code_dict, case_dict in zip(code_dataloader, case_dataloader):
            code_batch: DataProto = DataProto.from_single_dict(code_dict)
            case_batch: DataProto = DataProto.from_single_dict(case_dict)
            gen_code_batch = code_batch.pop(['input_ids', 'attention_mask', 'position_ids'])
            gen_case_batch = case_batch.pop(['input_ids', 'attention_mask', 'position_ids'])
            gen_code_batch.meta_info = {
                'eos_token_id': self.tokenizer.eos_token_id,
                'pad_token_id': self.tokenizer.pad_token_id,
                'recompute_log_prob': False,
                'do_sample': True,
            ...
            # pad to be divisible by dp_size
            gen_code_batch_padded, pad_size_code = pad_dataproto_to_divisor(gen_code_batch, self.actor_rollout_wg.world_size)
            gen_case_batch_padded, pad_size_case = pad_dataproto_to_divisor(gen_case_batch, self.actor_rollout_wg.world_size)
            output_gen_code_batch_padded = self.actor_rollout_wg.generate_sequences(gen_code_batch_padded)
            output_gen_case_batch_padded = self.actor_rollout_wg.generate_sequences(gen_case_batch_padded)
            pad_size_code *= self.config.actor_rollout_ref.rollout.n
            pad_size_case *= self.config.actor_rollout_ref.rollout.n

            # unpad
            output_gen_code_batch = unpad_dataproto(output_gen_code_batch_padded, pad_size=pad_size_code)
            output_gen_case_batch = unpad_dataproto(output_gen_case_batch_padded, pad_size=pad_size_case)

            # If we're doing multiple samples, repeat the original code_batch
            code_batch = code_batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
            case_batch = case_batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)

            code_batch = code_batch.union(output_gen_code_batch)
            case_batch = case_batch.union(output_gen_case_batch)

            # Store generated outputs for code
            output_ids = code_batch.batch['responses']
            output_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in output_ids]
            # code_batch.non_tensor_code_batch['generated_code'] = []
            for idx, output_text in enumerate(output_texts):
                code_output = self.extract_code(output_text)
                code_batch.non_tensor_batch['generated_code'][idx] = code_batch.non_tensor_batch['generated_code'][idx] + [code_output]

            # Store generated outputs for case
            output_ids = case_batch.batch['responses']
            output_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in output_ids]

            for idx, output_text in enumerate(output_texts):
                test_input, test_output, example_text = self.extract_test_cases(output_text)
                case_batch.non_tensor_batch["case_input"][idx] = case_batch.non_tensor_batch["case_input"][idx] + test_input
                # case_batch[index_i]["case_output"] = case_batch[index_i]["case_output"] + test_output
                # case_batch[index_i]["case_text"] = case_batch[index_i]["case_text"] + example_text
                # case_batch[index_i]["all_case_input"] = case_batch[index_i]["all_case_input"] + test_input
                # case_batch[index_i]["all_case_output"] = case_batch[index_i]["all_case_output"] + test_output
                # case_batch[index_i]["case_response_length"].append(case_response_length[i])
                # i += 1
        ...
        if self.loaded_datasets:
            ...
            if need_seed_dataset:
                if self.config.azr.seed_dataset is not None:
                    ...
                    random_select_num = min(random_select_num, len(seed_dataset))
                    seed_dataset = random.sample(seed_dataset, random_select_num)
        ...
        while self.global_steps < self.total_training_steps:
            ...
            data_len = data_len * 2 - 1  # prevent error in assert len(code_pred_train_dataloader) >= 1
            if 'code_i' in self.config.azr.problem_types:
